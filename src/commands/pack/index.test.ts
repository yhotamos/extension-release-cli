  ...
  return {
    default: () => {
      ...
      return {
        directory: vi.fn(),
        glob: vi.fn(),
        pipe: (o: NodeJS.WritableStream & NodeJS.EventEmitter) => { outputRef = o },
        finalize: () => {
          setImmediate(() => outputRef && outputRef.emit('close'))
        },
        on: vi.fn(),
...
describe('pack command', () => {
  ...
  it('requires source argument', async () => {
    ...
    packCommand(program)
    const exitSpy = vi.spyOn(process, 'exit').mockImplementation((code?: string | number | null | undefined): never => { throw new Error('process.exit:' + code) })
    await expect(program.parseAsync(['pack', '--releases-dir', 'some-dir'], { from: 'user' })).rejects.toThrow('process.exit:1')
  ...
  it('packs an existing source directory into a zip archive', async () => {
    ...
    try { fs.rmSync(releasesDir, { recursive: true, force: true }) } catch { }
  })
  ...
  it('dry-run does not create files and reports expected size', async () => {
    ...
    await program.parseAsync(['pack', tmpDir, '--releases-dir', releasesDir, '--dry-run'], { from: 'user' })
  ...
  it('refuses when releases directory is inside the source directory', async () => {
    ...
    await expect(program.parseAsync(['pack', tmpDir, '--releases-dir', releasesDir], { from: 'user' })).rejects.toThrow('process.exit:1')
    expect(exitSpy).toHaveBeenCalledWith(1)
...
describe('resolvePackZipFileName', () => {
  ...
  it('returns the custom name when --name is provided', async () => {
    ...
    const { zipFileName } = resolvePackZipFileName({ name: 'custom-out' }, 'src')
  ...
  it('throws an error when --name is invalid', async () => {
    ...
    vi.spyOn(manifestModule, 'getManifestData').mockReturnValue({ name: '', version: '1.2.3' })
    const { resolvePackZipFileName } = await import('./index')
    expect(() => resolvePackZipFileName({ name: 'My Extension!! 日本語' }, 'src')).toThrow(/invalid --name value/i)
  })
  ...
  it('uses the name from package.json when available (sanitized)', async () => {
    ...
    vi.spyOn(fs, 'readFileSync').mockImplementation(() => JSON.stringify({ name: 'pkg-name_123', version: '9.8.7' }))
  ...
  it('falls back to the manifest name as the last resort', async () => {
    const manifestModule = await import('../../utils/manifest')
    vi.spyOn(manifestModule, 'getManifestData').mockReturnValue({ name: 'MyManifest', version: '0.0.1' })
    vi.spyOn(fs, 'existsSync').mockReturnValue(false)
